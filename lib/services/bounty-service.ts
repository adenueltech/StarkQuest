...
// Contract instances
let StarkEarnMinimal: Contract | null = null;

// Initialize contracts
export const initializeContracts = () => {
  StarkEarnMinimal = new Contract(
    StarkEarnMinimalAbi,
    CONTRACT_ADDRESSES.StarkEarn_MINIMAL,
    provider
  );
};
...
export const createBounty = async (
  ...
  try {
    ...
    const { transaction_hash } = await account.execute({
      contractAddress: CONTRACT_ADDRESSES.StarkEarn_MINIMAL,
...
// Submit work
export const submitWork = async (bountyId: number, content: string) => {
  if (!account) {
    throw new Error("Wallet not connected");
  }
...
// Cancel bounty
export const cancelBounty = async (bountyId: number, reason: string) => {
  if (!account) {
    throw new Error("Wallet not connected");
  }
...
export const getBountyById = async (bountyId: number) => {
  try {
    ...
    const details = await StarkEarnMinimal?.get_bounty(bountyId);
    return details;
  } catch (error) {
    throw new Error(
...
export const getApplication = async (
  ...
  try {
    ...
    const application = await StarkEarnMinimal?.get_application(
      bountyId,
      applicationId
    );
    return application;
...
export const getSubmission = async (bountyId: number, submissionId: number) => {
  try {
    ...
    const submission = await StarkEarnMinimal?.get_submission(
      bountyId,
      submissionId
    );
    return submission;
...
export const getEscrowBalance = async (bountyId: number) => {
  try {
    ...
    const balance = await StarkEarnMinimal?.get_escrow_balance(bountyId);
    return balance;
  } catch (error) {
    throw new Error(
      `Failed to get escrow balance: ${(error as Error).message}`
    );
  }
};
...
export const getAllBounties = async () => {
  try {
    ...
    // Get bounty count
    const count = await StarkEarnMinimal?.get_bounty_count();
