// Simple test deployment script for StarkEarn
...
async function testDeploy() {
  ...
  if (!ownerAddress || !privateKey) {
    ...
    console.log(
      "OWNER_ADDRESS and PRIVATE_KEY must be set in .env file for actual deployment"
    );
    return;
  }
  ...
  try {
    ...
    const contractPath = path.resolve(
      __dirname,
      "../target/dev/StarkEarn_HelloStarknet.contract_class.json"
    );
    const casmPath = path.resolve(
      __dirname,
      "../target/dev/StarkEarn_HelloStarknet.compiled_contract_class.json"
    );
    ...
    console.log(
      `   Contract declared successfully with class hash: ${classHash.class_hash}`
    );
    ...
    console.log("\nNext steps:");
    console.log(
      "1. Fix the individual contract files to generate proper CASM files"
    );
    console.log(
      "2. Update environment variables with real StarkNet credentials"
    );
